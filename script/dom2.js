...
*/
/* 
변수 선언 시 인덱스를 작성하고 선언했다면 변수 사용 시 인덱스를 붙이지 않고 변수명만 간편하게 쓸 수 있다. 대신, 인덱스가 고정되므로 다른 인덱스 번호에 해당하는 태그나 클래스를 사용하고 싶을때 변수를 새로 만들어야 한다.
console.log(aTag2);
...
//const gnb = headre[0].getElementById('gnb')
//console.log(gnb)
//float, flex, grid | getelement | queryslector
console.log('---------------------')
const headerTag = document.querySelector('header')
console.log(headerTag)
//querySelecto는 tag, class를 선택 시 메서드 괄호안 문자열 대상 기준으로 가장 먼저 인식된 대상을 출력해준다.
...
console.log(typeof input_text) //=>object
console.log(typeof input_num) //=>object
console.log(typeof btn) //=>object */
    console.log(input_text.value)
    console.log(typeof input_text.value) //=>string
    console.log(input_num.value)
    console.log(typeof input_num.value) //=>string
    console.log(typeof Number(input_num.value)) //=>number
}
function btn_func2(){
...
//객체.속성 = 속성에 대입할 값;
function btn_func3(){
//사용자가 요청사항을 입력 후 주문 버튼을 클리하면
//윈도우 경고창으로 다음과 같이 출력하기
//"사용자 요청사항은 oooo 입니다."
    let total = input_text.value
    window.alert(`사용자 요청사항은 ${total} 입니다.`)
    //console.log(total)
}
function btn_func4(){
...
//제품 1개당 1000원 기준
    let memo = input_text.value; //요청사항 저장
    let nom = Number(input_num.value); //수량 저장
    let price = 1000
    //let total = nom*price
    window.alert(`요청사항은 ${memo}, 주문수량은 ${nom}개, 총 결제 금액은 ${nom*price/* total */}원 입니다.`)
}