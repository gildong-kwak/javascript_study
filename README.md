240514~현재진행(ing)
# javascript
* 자바스크립트는 절자적언어, 객체지향언어, 함수형 언어, 논리형 언어이다.
...
### 변수 OX 퀴즈
1. `var 1num = 10;` (X) 숫자가 앞에 들어가서
2. `var $num = 10;` (O) 
3. `var 100num = 10;` (X) 숫자가 앞에 들어가서
4. `var num100 = 10;` (O)
5. `var console = 10;` (X) 콘솔은 객체명으로 사용 안된다
6. `var num = 10;` (O)
7. `var log = hello;` (X) 로그는 예약어로 사용 안된다, 문자에 따옴표 안들어감
8. `var Num = 10;` (O) 대문자 사용 시 쭉 대문자 사용
9. `var num = js;` (X) 문자에 따옴표 안들어감
10. `var js = num;` (X) 문자에 따옴표 안들어감
## 함수 (function)
...
4. 중괄호 `{}` 내에 재사용되는 반복 프로그래밍 코드 작성하기
...
* 함수의 지역변수로 생성 시 서로 다른 함수에서 해당 지역변수 이름을 중복하용할 수 있다.
* `function fun1(){ let a=1; }`
* `function fun2(){ let a=1; }`
## 배열
* 2개 이상 데이터를 저장할 때 배열을 사용해야 한다.
* `const(var ,let 등) 배열명 = new Array(데이터1, 데이터2)`
